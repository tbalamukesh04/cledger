...
def run_date_verb_test_scenario(scenario_name: str, message_text: str, expected_verb: str, expected_date_offset_days: int):
    ...
    try:
        ...
        actual_date = transaction.txn_date.date()
        
        assert actual_verb == expected_verb, f"Expected Verb '{expected_verb}', got '{actual_verb}'"
        
        expected_full_date = (datetime.now(timezone.utc) + timedelta(days=expected_date_offset_days)).date()
        assert actual_date == expected_full_date, f"Expected Date '{expected_full_date}', got '{actual_date}'"
            
    finally:
        db.close()

def test_date_verb_extraction_scenarios():
    setup_logging()
