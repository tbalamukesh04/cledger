...
def test_valid_response_accepted():
    ...
    result = parse_batch_response(raw_response, ["1"], "batch-test-01")

    assert result["1"] is not None
    assert result["1"].amount == 500.0
    assert result["1"].currency == "INR"
    assert result["1"].transaction_verb == "debit"
    assert result["1"].transaction_date == "2026-03-10"

def test_missing_fields_rejected():
    # Omitting the strictly required "amount" field to force a validation failure
    missing_fields_json = """
    [
      {
        "id": 2,
...
def test_incorrect_data_types_rejected():
    ...
    result = parse_batch_response(raw_response, ["3"], "batch-test-03")
...
def test_malformed_json_raises_error():
    ...
    with pytest.raises(ValueError, match="Malformed JSON response from LLM"):
        parse_batch_response(raw_response, ["4"], "batch-test-04")