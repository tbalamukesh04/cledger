...
def test_correct_action_and_audit(mock_audit, mock_correct_svc, mock_get_txn, mock_review_needed_txn):
    ...
    updated_txn.raw_message = None  # Bypass Pydantic nested extraction    
...
# ---------------------------------------------------------
@patch("app.api.transactions.get_transaction_by_id")
def test_invalid_status_rejection(mock_get_txn):
    ...
    # Should reject the review action
    assert response.status_code == 400
    assert "not in REVIEW_NEEDED state" in response.json()["detail"]
