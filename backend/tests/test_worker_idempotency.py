...
def run_idempotency_tests():
    ...
    # =====================================================================
    # TEST CASE 1: Duplicate WAMID (Caught by Router)
    # =====================================================================
    print(f"{'='*60}\n🚀 SCENARIO 1: Exact WAMID Duplicate\n{'='*60}")
    ...
    print("\n-> Sending DUPLICATE Webhook (Same WAMID)...")
    trigger_webhook(payload_1)
    job_1b = get_job_from_queue()
    
    if job_1b is None:
        print("✅ SCENARIO 1 PASSED: Webhook Router blocked the duplicate WAMID before it ever reached Redis.\n")

    # =====================================================================
    # TEST CASE 2: Duplicate Content Hash (Caught by Worker)
    # =====================================================================
    print(f"{'='*60}\n🚀 SCENARIO 2: Content Hash Duplicate (No WAMID)\n{'='*60}")
    ...
    print("-> Processing Duplicate Job (Watch for Worker Abort Log)...")
    if job_2b:
        success = process_webhook_job(job_2b)
        if success:
            print("✅ SCENARIO 2 PASSED: Worker content hash duplicate was safely aborted.\n")

    # =====================================================================
    # TEST CASE 3: Worker retry of previously processed job
    # =====================================================================
    print(f"{'='*60}\n🚀 SCENARIO 3: Worker Job Retry\n{'='*60}")
    ...
    print("-> Processing Job Retry (Watch for Abort Log)...")
    success = process_webhook_job(job_3)
    if success:
        print("✅ SCENARIO 3 PASSED: Worker recognized the job was already done and safely aborted.\n")
