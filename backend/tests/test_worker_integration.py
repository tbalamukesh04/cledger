...
def test_worker_transaction_persistence_integration():
    ...
    try:
        ...
        # --- 6. CLEANUP ---
        # Ensure test leaves no lingering data in the development DB
        txn_to_delete = db.query(Transactions).filter(Transactions.raw_message_id == raw_msg.id).first()
        if txn_to_delete:
            db.delete(txn_to_delete)
        db.delete(raw_msg)
        db.commit()
        db.close()