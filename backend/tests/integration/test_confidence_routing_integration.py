...
def test_confidence_routing_high_confidence(db_session):
    ...
    with patch("app.workers.job_handler.parse_batch_response", return_value={str(raw_msg.id): mock_extraction}):
    ...
    transaction = db_session.query(Transactions).filter_by(raw_message_id=raw_msg.id).first()
    ...
    assert meta.get("routing_status") == "accepted"
...
def test_confidence_routing_low_confidence(db_session):
    ...
    # Verify metadata routing details
    meta = transaction.parsing_meta.get("ai_extraction", {})
    assert meta.get("routing_status") == "review_required"
